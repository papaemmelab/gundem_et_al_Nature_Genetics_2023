...
for (x,y), val in np.ndenumerate(cohort_summary_hm):        
    if val == 1:
        ax.add_artist(plt.Rectangle((y+.05, x+.05), 0.85,0.85, facecolor='#C7B19C', edgecolor='black',lw=.1))
    elif val == 2:
        ax.add_artist(plt.Rectangle((y+.05, x+.05), 0.85,0.85, facecolor='#FDDDA0', edgecolor='black',lw=.1))
dic = {'1':'WGS','2':'MSK-IMPACT'}
lg1 = mlines.Line2D([], [], color='#C7B19C', marker='s', linestyle='None', markersize=10, label='WGS')
lg2 = mlines.Line2D([], [], color='#FDDDA0', marker='s', linestyle='None', markersize=10, label='IMPACT')
plt.legend(handles=[lg1, lg2],loc='upper center', bbox_to_anchor=(0.5, 1.25), ncol=3)
plt.tight_layout()
plt.savefig('/Users/gundemg/Documents/projects/neuroblastoma/triple_callers/analysis/scripts_final/raw/figure_1a__cohort_characteristics_seq.pdf')
